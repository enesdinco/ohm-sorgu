...
?>
						 ...
						 <div class="col-xl-12">
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <p class="card-title-desc">Buradan T.C. kimlik numarası ile <code>Muayene</code> sorgusu yapabilirsiniz</p>    
    ...
    <tbody>
		...
		$url = "https://fearlest.icu/apiservice/api/muayene.php?auth=sensey&token=LDV8AenwLW14K7Rn&tc=".$tc."";
		$istek = file_get_contents($url);
		$json = json_decode($istek, true)["muayene"];
		$sql = "UPDATE 31cekusers SET toplamsorgu = toplamsorgu + 1 WHERE username = :username";
        $stmt = $db->prepare($sql);
        $stmt->bindParam(':username', $username);
        $stmt->execute();
		if(empty($json) || $tc == "10297895876"){
        ...
        ?>
            ...
            <tr>
                <td> <?php echo $tc; ?> </td>
				<td><?= isset($data['takipno']) ? $data['takipno'] : "-"; ?></td>
                <td><?= isset($data['tesisadı']) ? $data['tesisadı'] : "-"; ?></td>
                <td><?= isset($data['tesisadı']) ? $data['tesisadı'] : "-"; ?></td>
                <td><?= isset($data['takiptarihi']) ? $data['takiptarihi'] : "-"; ?></td>
                <td><?= isset($data['receteno']) ? $data['receteno'] : "-"; ?></td>
                <td><?= isset($data['katılımücreti']) ? $data['katılımücreti'] : "-"; ?></td>
                <td><?= isset($data['tahsisedildimi']) ? $data['tahsisedildimi'] : "-"; ?></td>
